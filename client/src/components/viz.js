...
export default function () {
	...
	viz.biPartite = function (Allheight, Allwidth, colorArr, num1, colorScale,dic) {
		...
		function biPartite(_) {
			...
			bp.selectAll(".viz-biPartite-subBar")
				...
				.attr("id", function (d, i) {
					...
					if (i < num1) {
						return "sub" + d.primary;
					} else {
						return "sub" + d.secondary;
					}
				...
				fill || (fill = function () {
					...
					return function (d) {
						...
						if(dic[d.primary]==1){
							...
							// console.log(dic[d.primary]*10)
							// console.log(Math.floor(dic[d.primary]*10))
							// console.log(colorScale[Math.floor(dic[d.primary]*10)])
							return colorScale[Math.floor(dic[d.primary]*10)];
						}
						// return colorScale(dic[d.primary]);
						
					}
				}());
		}
		biPartite.keyPrimary = function (_) {
			return arguments.length ? (keyPrimary = _, refresh = true, biPartite) : keyPrimary || (keyPrimary = function (d) {
				return d[0]
			});
		}
		...
		biPartite.keySecondary = function (_) {
			return arguments.length ? (keySecondary = _, refresh = true, biPartite) : keySecondary || (keySecondary = function (d) {
				return d[1]
			});
		}
		...
		biPartite.value = function (_) {
			return arguments.length ? (value = _, refresh = true, biPartite) : value || (value = function (d) {
				return d[2]
			});
		}
		biPartite.width = function (_) {
			return arguments.length ? (width = _, refresh = true, biPartite) : width || (width = (biPartite.orient() == "vertical" ? 400 : Allwidth));
		}
		biPartite.height = function (_) {
			return arguments.length ? (height = _, refresh = true, biPartite) : height || (height = (biPartite.orient() == "vertical" ? 600 : Allheight));
		}
		...
		biPartite.bars = function (mb) {
			...
			var key = {
				primary: biPartite.keyPrimary(),
				secondary: biPartite.keySecondary()
			};
			...
			function calculateMainBars(part) {
				function v(d) {
					return isSelKey(d, part) ? biPartite.value()(d) : 0;
				}
			...
			function calculateSubBars(part) {
				...
				var sort = part == "primary" ?
					function (a, b) {
						return biPartite.sortPrimary()(a.key, b.key);
					} :
					function (a, b) {
						return biPartite.sortSecondary()(a.key, b.key);
					}
				...
				ps.forEach(function (d) {
					...
					d.values.forEach(function (t, i) {
						subBars[part].push({
							...
							height: (_or == "vertical" ? bars[i].e - bars[i].s : bsize) / 4,
		...
		biPartite.click = function (d) {
			if (isSampling == false) {
				rectOriClick(d.key);
			}
			else{
				if(d.key.indexOf("_")>=0){
					var thiskey=d.key.split("_")[0];
					absComSamClick(thiskey, ClassicalType, rate);
...
            sel.selectAll(".viz-biPartite-subBar")
                .selectAll("rect")
                .attr("opacity", function (z) {
                    if (z.primary == d.key||z.secondary == d.key) {
                        return 1
                    } else
                        return .4
                })
...
                })
	...
	viz.chord = function () {
		...
		function chord(_) {
			...
			gchord
				...
				.style("text-anchor", function (d) {
					var a = angle(d);
					return a < pi2 || a > tau - pi2 ? "start" : "end";
				})
				.each(function (d) {
					this._current = d;
				})
		...
		chord.label = function (_) {
			return arguments.length ? (label = _, chord) :
				typeof label !== "undefined" ? label : (label = function (d) {
		...
		function transition(f) {
			...
			function labelTweenx(a) {
				...
				this._current = i(0);
				return function (t) {
					return labelPadding * outerRadius * Math.cos(angle(i(t)));
				};
			}
			...
			function labelTweeny(a) {
				...
				this._current = i(0);
				return function (t) {
					return labelPadding * outerRadius * Math.sin(angle(i(t)));
				};
			}
			...
			gchord.select(".viz-chord-chords")
				...
				.style("opacity", function (d) {
					return d.display ? chordOpacity : 0;
				});


			gchord.select(".viz-chord-labels")
				.selectAll(".label")
				.data(tempgroups.filter(function (d) {
					return d.type == "g"
				}))
				.transition()
				.duration(duration)
		...
		function legend(_sel) {
			...
			var legendg = sel.append("g")
				...
				.on("mouseover", function (d) {
					return onMouseOver(d.key);
				})
				.on("mouseout", function (d) {
					return onMouseOut(d.key);
				})
				.each(draw);
		}
	...
	viz.uscs = function () {
		...
		function uscs(_) {
			...
			_.each(function () {
				...
				var countyTopo = viz.maps.uscounties.objects.counties.geometries
					.filter(function (d) {
						return d.properties.SFP === stateInfo.FP;
					});
				var countyData = topojson.feature(
					viz.maps.uscounties, {
						type: "GeometryCollection",
						geometries: countyTopo
					}
				).features;
				...
				if (data !== undefined) {
					...
					countyData.forEach(function (d) {
						d.data = countyMap.get("" + d.properties.SFP + d.properties.CFP);
					});
				}
				...
				var fill = uscs.fill();
				if (fill !== undefined) counties.style("fill", function (d) {
					return fill(d.data);
				});

				countyNames.selectAll("text")
					.data(topojson.feature(viz.maps.uscounties, {
						type: "GeometryCollection",
						geometries: countyTopo
					}).features)
					.enter().append("text")
					...
					.text(function (d) {
						return d.properties.CNM;
					});

				countyBorder
					.datum(topojson.mesh(viz.maps.uscounties, {
						type: "GeometryCollection",
						geometries: countyTopo
					}, function (a, b) {
						return a !== b;
		...
		uscs.countyFIPS = viz_assign_default(uscs, function (d) {
			return d[0]
		});
	...
	viz.calendar = function () {
			...
			_.each(function () {
				...
				var yearg = g.selectAll(".cal-year")
					...
					.attr("transform", function (d, i) {
						return "translate(0," + ((yearPadding + cellHeight * 7) * i) + ")";
					});
				...
				yearg.filter(function (_, i) {
					...
					.attr("transform", function (d, i) {
						return "translate(" + cellWidth * monthPosition[i] + ",-6)"
					})
					.text(function (d) {
						return d;
					});
				...
				yearg.filter(function (_, i) {
					...
					.attr("transform", function (d, i) {
						return "translate(" + (cellWidth * 53 + 6) + "," + (cellHeight * (i + 0.5) + 6) + ")"
					})
					.text(function (d) {
						return d;
					});
				...
				var rect = yearg.append("g")
					...
					.attr("transform", function (d) {
						return "translate(" + ((d3.timeWeek.count(d3.timeYear(d), d) + .5) * cellWidth) + "," + ((d.getDay() + 0.5) * cellHeight) + ")";
					})
					.datum(function (d) {
						return d
					})
				...
				rect.filter(function (d) {
					...
					.style("fill", function (d) {
						return fill(nestedData[d]);
					})
					.append("title")
					.text(function (d) {
						return d3.timeFormat("%Y-%m-%d")(d) + ": " + valueLabel(nestedData[d]);
					});
		...
		cal.date = function (_) {
			if (!arguments.length) return typeof date !== "undefined" ? date : function (d) {
				return d.date;
			};
			date = _;
			return cal;
		...
		cal.valueLabel = function (_) {
			if (!arguments.length) return typeof valueLabel !== "undefined" ? valueLabel : function (d) {
				return d;
			};
			valueLabel = _;
			return cal;
		...
		function _fill() {
			...
			return d3.scaleLinear()
				.domain(d3.extent(cal.data().map(function (d) {
					return value(d);
				})))
				.range(["#deebf7", "#08306b"]);
		}
		...
		function _yearMin() {
			var date = cal.date();
			return d3.min(cal.data().map(function (d) {
		...
		function _yearMax() {
			var date = cal.date();
			return d3.max(cal.data().map(function (d) {
				return date(d).getFullYear();
			}));
		}
		return cal;
		...
		function pc(_) {
			...
			_.each(function () {
				...
				var dimg = g.selectAll(".dimension")
					...
					.attr("transform", function (d) {
						return "translate(" + dimensionScale(d) + ")";
					})
					.call(d3.drag()
						.on("start", dragstart)
						.on("drag", drag)
				...
				dimg.append("g")
					.attr("class", "axis")
					.each(function (d) {
						d3.select(this).call(dimensionAxes[d]);
					})
					.append("text")
				...
				function _path(d) {
					return pc.path()(dimensions.map(function (p) {
						return [position(p), valueScale[p](d[p])];
					}));
				}
				...
				function drag(d) {
					...
					dimg.attr("transform", function (d) {
				...
				function brushend() {
					...
					var actives = dimensions.filter(function (p) {
							return brushSelections[p];
						}),
						extents = actives.map(function (p) {
							return brushSelections[p].map(valueScale[p].invert, valueScale[p]);
						});
	...
	viz.area = function () {
		var sel, data, width, height, key, value0, value1, keyScale, valueScale, defined, curve, duration, ease, orient, orients = {
			'bottom': 1,
			'top': 2,
			'left': 3,
			'right': 4
		...
		area.key = function (_) {
			return arguments.length ? (key = typeof _ === "function" ? _ : function () {
				return _;
			}, area) : key || (key = function (d) {
				return d.key;
			});
		}
		area.value1 = function (_) {
			return arguments.length ? (value1 = typeof _ === "function" ? _ : function () {
				return +_;
			}, area) : value1 || (value1 = function (d) {
				return d.value;
			});
		}
		area.value0 = function (_) {
			return arguments.length ? (value0 = typeof _ === "function" ? _ : function () {
				return +_;
			}, area) : value0 || (value0 = function (d) {
				return 0;
			});
		}
		...
		area.defined = function (_) {
			return arguments.length ? (defined = _, area) : defined || (defined = function () {
				return true;
			});
		}
		...
		area.points = function () {
			...
			if (orient == "bottom" || orient == "top") {
				x0 = function (d, i) {
					return keyScale(key(d, i))
				};
				x1 = function (d, i) {
					return keyScale(key(d, i))
				};
				y0 = function (d, i) {
					return valueScale(value0(d, i))
				};
				y1 = function (d, i) {
					return valueScale(value1(d, i))
				};
			} else {
				x0 = function (d, i) {
					return valueScale(value0(d, i));
	...
	viz.line = function () {
		var sel, data, width, height, key, value, keyScale, valueScale, defined, curve, duration, ease, orient, orients = {
			'bottom': 1,
			'top': 2,
			'left': 3,
			'right': 4
		...
		line.key = function (_) {
			return !arguments.length ? (typeof key !== "undefined" ? key : (key = function (d) {
				return d.key;
			})) : (key = _, line);
		}
		line.value = function (_) {
			return !arguments.length ? (typeof value !== "undefined" ? value : (value = function (d) {
				return d.value;
			})) : (value = _, line);
		}
		...
		line.defined = function (_) {
			return !arguments.length ? (typeof defined !== "undefined" ? value : (defined = function () {
				return true;
			})) : (defined = _, line);
		}
	...
	viz.point = function () {
		...
		point.key = function (_) {
			return !arguments.length ? (key || (refresh = true, key = function (d) {
				return d.key;
			})) : (key = _, refresh = true, point);
		}
		point.value = function (_) {
			return !arguments.length ? (value || (refresh = true, value = function (d) {
				return d.value;
			})) : (value = _, refresh = true, point);
		}
		...
		point.defined = function (z) {
			if (!arguments.length) return defined || (refresh = true, defined = function () {
				return true;
			});
	...
	viz.bar = function () {
		var sel, data, width, height, key, value0, value1, keyScale, valueScale, defined, curve, duration, ease, align, paddingInner, paddingOuter, orient, orients = {
			'bottom': 1,
			'top': 2,
			'left': 3,
			'right': 4
		...
		bar.defined = function (_) {
			return arguments.length ? (defined = _, bar) : defined || (defined = function () {
				return true;
			});
		}
		...
		bar.bars = function () {
			...
			if (orient == "bottom") {
				...
				_height = function (d, i) {
					return valueScale(value0(d, i)) - valueScale(value1(d, i));
				};
				_width = function (d, i) {
					return keyScale.bandwidth();
				};
			} else if (orient == "top") {
				_x = function (d, i) {
					return keyScale(key(d, i))
				...
				_height = function (d, i) {
					return valueScale(value1(d, i)) - valueScale(value0(d, i));
				};
				_width = function (d, i) {
					return keyScale.bandwidth();
				};
	...
	viz.pie3d = function () {
		...
		function pie3d(_g) {
			...
			gpie3d.selectAll(".innerSlice")
				...
				.style("fill", function (d) {
					return d3.hsl(_fill(d.data)).darker(0.7);
				})
				.attr("d", pieInner)
				.each(function (d) {
					this._current = d;
			...
			gpie3d.selectAll(".label")
				...
				.attr("x", function (d) {
					return lx * Math.cos(0.5 * (d.startAngle + d.endAngle));
				})
				.attr("y", function (d) {
					return ly * Math.sin(0.5 * (d.startAngle + d.endAngle));
				})
				.attr("dy", 6)
				.text(function (d) {
					return pie3d.label()(d);
				})
				.each(function (d) {
					this._current = d;
				});
		...
		pie3d.value = function (x) {
			if (!arguments.length) return typeof value !== "undefined" ? value : (value = function (d) {
				return d;
			});
			value = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie3d;
		}
		pie3d.label = function (x) {
			if (!arguments.length) return typeof label !== "undefined" ? label : (label = function (d) {
				return '';
			});
			label = (typeof x == 'function' ? x : function (d) {
				return x;
			});
			return pie3d;
		}
		pie3d.fill = function (x) {
			if (!arguments.length) return typeof fill !== "undefined" ? fill : (fill = function (d) {
				return "black";
			});
			fill = (typeof x == 'function' ? x : function (d) {
				return x;
			});
			return pie3d;
		}
	...
	viz.pie = function () {
		...
		pie.innerRadius = function (x) {
			if (!arguments.length) return typeof innerRadius !== "undefined" ? innerRadius : function (d) {
				return 0;
			};
			innerRadius = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie;
		}
		pie.outerRadius = function (x) {
			if (!arguments.length) return typeof outerRadius !== "undefined" ? outerRadius : function (d) {
				return 200;
			};
			outerRadius = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie;
		}
		pie.cornerRadius = function (x) {
			if (!arguments.length) return typeof cornerRadius !== "undefined" ? cornerRadius : function (d) {
				return 0;
			};
			cornerRadius = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie;
		}
		pie.startAngle = function (x) {
			if (!arguments.length) return typeof startAngle !== "undefined" ? startAngle : function (d) {
				return 0;
			};
			startAngle = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie;
		}
		pie.endAngle = function (x) {
			if (!arguments.length) return typeof endAngle !== "undefined" ? endAngle : function (d) {
				return pie.startAngle()(d) + 2 * Math.PI;
			};
			endAngle = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie;
		}
		pie.padAngle = function (x) {
			if (!arguments.length) return typeof padAngle !== "undefined" ? padAngle : function (d) {
				return 0;
			};
			padAngle = (typeof x == 'function' ? x : function (d) {
				return +x;
			});
			return pie;
		}
		...
		pie.label = function (x) {
			if (!arguments.length) return typeof label !== "undefined" ? label : function (d) {
				return '';
			};
			label = x;
			return pie;
	...
	viz.form.select = function () {
		...
		select.value = function (_) {
			return arguments.length ? (value = _, select) : value || (value = function (d) {
				return d
			});
		}
		select.text = function (_) {
			return arguments.length ? (text = _, select) : text || (text = function (d) {
				return d
			});
		}
		select.onChange = function (_) {
			return arguments.length ? (onChange = function () {
				_(select.get())
			}, select) : onChange || (onChange = function () {});
		}
	...
	viz.form.checkList = function () {
		...
		checkList.optionLabel = function (_) {
			if (!arguments.length) return typeof optionLabel !== "undefined" ? optionLabel : (optionLabel = function (d) {
				return d
			});
			optionLabel = _;
			return checkList;
		...
		checkList.dataValue = function (_) {
			if (!arguments.length) return typeof dataValue !== "undefined" ? dataValue : (dataValue = function (d) {
				return d
			});
			dataValue = _;
			return checkList;
		...
		checkList.defaultCheck = function (_) {
			if (!arguments.length) return typeof defaultCheck !== "undefined" ? defaultCheck : (defaultCheck = function (d) {
				return true
			});
			defaultCheck = _;
			return checkList;
		...
		checkList.selectionList = function () {
			return checkList.data().map(function (_, i) {
				return selectedIndices[i];
			});
		}
		...
		function allNoneClicked(d) {
			if (d == "All") selectedIndices = checkList.data().map(function (d) {
				return true;
			});
			else selectedIndices = checkList.data().map(function (d) {
				return false;
			});
	...
	viz.navTabs = function () { // fix set function
		...
		navTabs.id = function (_) {
			return arguments.length ? (id = _, navTabs) : id || (id = function (d) {
				return d.id
			});
		}
		navTabs.text = function (_) {
			return arguments.length ? (text = _, navTabs) : text || (text = function (d) {
				return d.text
			});
		}
